  ...
  return (
    ...
    >
      ...
      <div className="flex items-center ">
        <h1 className="px-4 py-4 mr-auto text-lg font-bold text-zinc-200">
          {props.name}
        </h1>
        <div
        ...
        <Switch
          checked={doc.network}
          onClick={doc.setNetwork.bind(null, !doc.network)}
        />
      </div>
      ...
      <div className="mt-4 overflow-hidden border border-dashed rounded-md border-zinc-800 min-h-[100px] max-h-[100px] overflow-y-auto">
        {doc.staging.length ? (
          <div className="flex flex-wrap gap-2 p-2">
            {doc.staging.map((operation, i) => {
              return (
                ...
                >
                  <div
                    className={`shadow-lg ${operation.type == 'insert' ? 'bg-green-400/20 text-green-400' : 'bg-red-400/20 text-red-400'} flex items-center justify-center w-4 h-4 text-lg bg-white rounded-full`}
                  >
                  ...
                  <span className="ml-2">
                    {`${operation.id}`}{' '}
                    {operation.type == 'insert' ? `'${operation.value}'` : ''}
                  </span>
                </div>
              );
            })}
          ...
          <div className="flex items-center justify-center w-full h-[100px] py-4 font-bold text-center">
            Staging
          </div>
        )}
      </div>
